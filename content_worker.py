...
def update_counts(connection) -> None:
    counts = {
...
def process_batch(
    ...
    rows = next_pending(connection, batch_size)
    if rows:
        set_runtime_state(
            connection,
            "indexer_current",
    ...
    with ThreadPoolExecutor(max_workers=max(1, workers)) as executor:
        ...
        for future in as_completed(futures):
            row = futures[future]
            try:
                apply_enrichment(connection, future.result())
